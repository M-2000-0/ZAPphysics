# ═══════════════════════════════════════════════════════════════════
# ZapPhysics — World3D: 3D Simulation container and integrator
# ═══════════════════════════════════════════════════════════════════
...
class World3D:
  ...
  fn apply_global_forces(self)
    for p in self.particles:
      if self.has_gravity:
        let weight = Vec3(self.gx, self.gy, self.gz).scale(p.mass)
        p.apply_force(weight)

  fn apply_gravity_forces(self)
    for i in range(len(self.particles)):
      for j in range(i + 1, len(self.particles)):
        let f = gravity3d(self.particles[i], self.particles[j], self.g_const)
  ...
  fn total_energy(self)
    ...
    for i in range(len(self.particles)):
      for j in range(i + 1, len(self.particles)):
        pe = pe + gravity3d_potential(self.particles[i], self.particles[j], self.g_const)
    ke + pe
  ...
  fn center_of_mass(self)
    ...
    for p in self.particles:
      ...
      mz = mz + p.pos.z * p.mass
      total_m = total_m + p.mass
    if total_m > 0:
      Vec3(mx / total_m, my / total_m, mz / total_m)
    el:
      Vec3(0, 0, 0)
  ...
  fn summary(self)
    ...
    say("  center of mass: (" + str(round(com.x, 2)) + ", " + str(round(com.y, 2)) + ", " + str(round(com.z, 2)) + ")")
    let mom = self.momentum_sum()
    say("  total momentum: (" + str(round(mom.x, 4)) + ", " + str(round(mom.y, 4)) + ", " + str(round(mom.z, 4)) + ")")
    for p in self.particles:
      say("  " + p.name + " pos=(" + str(round(p.pos.x, 2)) + "," + str(round(p.pos.y, 2)) + "," + str(round(p.pos.z, 2)) + ")"
          + " vel=(" + str(round(p.vel.x, 2)) + "," + str(round(p.vel.y, 2)) + "," + str(round(p.vel.z, 2)) + ")"
